...
input int SLPoint=5000;//損切り幅（ポイント）
input int TPPoint=2500;//利食い幅（ポイント）
...
void OnTick(){
  ...
  double BBLower1=iBands(_Symbol,0,BBPeriod,BBDeviation,0,PRICE_CLOSE,MODE_LOWER,1);//２本前のボリンジャーバンド
  ...
  if(Close[2]>=BBLower2&&Close[1]<BBLower1)//買いシグナル
  {
    ...
    //ポジションがなければ買い注文
    if(pos==0)Ticket=OrderSend(_Symbol,OP_BUY,Lots,Ask,0,Ask-SLPoint*_Point,Ask+TPPoint*_Point);
  }
  if(Close[2]<=BBUpper2&&Close[1]>BBUpper1)//売りシグナル
  {
    ...
    //ポジションがなければ売り注文
    if(pos==0)Ticket=OrderSend(_Symbol,OP_SELL,Lots,Bid,0,Bid+SLPoint*_Point,Bid-TPPoint*_Point);
  }
}
